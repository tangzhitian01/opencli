...
function getAkSk(): string | null {
  return process.env.WE_MP_RSS_AK_SK || null;
}

function buildHeaders(akSk?: string | null): Record<string, string> {
...
async function apiFetch(page: any, path: string, akSk?: string | null, options: RequestInit = {}): Promise<unknown> {
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const list = (result as any)?.list || (result as any)?.data || [];
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const result = await apiFetch(
      page,
      `/api/mps?limit=${kwargs.limit}&status=${status}`,
      akSk,
    );
    const list = (result as any)?.list || (result as any)?.data || [];
    return list.map((mp: any) => ({
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    if (kwargs.search) path += `&search=${encodeURIComponent(String(kwargs.search))}`;
    if (kwargs.only_favorite) path += '&only_favorite=true';
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const body: Record<string, string> = { mp_name: String(kwargs.mp_name) };
    if (kwargs.mp_id) body['mp_id'] = String(kwargs.mp_id);
    ...
    return [{
      ...
      status: '添加失败',
      message: (mp as any)?.message || (mp as any)?.errorMsg || JSON.stringify(mp),
    }];
  },
});
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const result = await apiFetch(
      page,
      `/api/mps/update/${kwargs.mp_id}?start_page=0&end_page=1`,
      akSk,
    );
...
cli({
  ...
  func: async (page, kwargs) => {
    ...
    const result = await apiFetch(
      page,
      `/api/rss?limit=${kwargs.limit}`,
      akSk,
    );
    const list = (result as any)?.list || (result as any)?.data || [];
    const baseUrl = getBaseUrl();
    return list.map((feed: any) => ({
