...
| **[we-mp-rss](./browser/we-mp-rss)** | `search` `subs` `articles` `add` `refresh` `tags` `rss` | 🔐 Browser + `WE_MP_RSS_URL` + `WE_MP_RSS_AK_SK` |
| **[we-mp-rss-sync](./browser/we-mp-rss-sync)** | `sync` `list` `add` `articles` | 本地 CLI + `WE_MP_RSS_URL` + `WE_MP_RSS_AK_SK` |
